...
const getSpots = (
  isIded?: boolean
): SeatChart<{ player: Player; seat: Seat }> => {
...
const arrangeGame = async (
  curator: Curator,
  options?: {
    isIded?: { spots: boolean };
    withBlinds?: Blind[];
    withPockets?: Pocket[];
  }
) => {
  const params = {
    ...
    pockets: (() => {
      const pockets = new SeatChart<Pocket>();
      if (options?.withPockets)
        options.withPockets.forEach((pocket, seat) =>
          pockets.write(seat, pocket)
        );
      return pockets;
    })(),
  };
...
describe("Curator", () => {
  ...
  describe("arrangeGame", () => {
    ...
    it("should throw error if given blind not associated to a seat", async () => {
      await expect(() =>
        arrangeGame(curator, { withBlinds: [<Blind>{ seat: 2 }] })
      ).rejects.toThrow(new UndefinedSeatError(2));
    });

    it("should throw error if given pocket not associated to a seat", async () => {
      const pockets: Pocket[] = [];
      pockets[2] = <Pocket>{ seat: 2 };
      await expect(() =>
        arrangeGame(curator, { withPockets: pockets })
      ).rejects.toThrow(new UndefinedSeatError(2));
    });
  });
  ...
  describe("present action", () => {
    ...
    it("should throw error if board is not defined", async () => {
      const action = <Action>Object.assign(getPublisher(), { seat: 0 });
      await expect(() => curator.presentAction(action)).rejects.toThrow(
        new UndefinedBoardError()
      );
    });
  ...
  describe("board", () => {
    ...
    it("should fail to collect if board is not defined", () => {
      const card = { rank: 0, suit: 0 };
      expect(() => curator.collectBoard(card)).toThrow(
        new UndefinedBoardError()
      );
    });

    it("should fail to present if game is not defined", async () => {
      await expect(() => curator.presentBoard()).rejects.toThrowError(
        new UndefinedGameError()
      );
    });
  });
  ...
  describe("exhibitGame", () => {
    it("should succeed", async () => {
      ...
      const { game, table } = await arrangeGame(curator, {
        isIded: { spots: true },
      });
      game.end = jest.fn();
