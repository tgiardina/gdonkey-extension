...
import { InactiveSeatError, UndefinedButtonError, UndefinedCasinoError, UndefinedGameError, UndefinedPlayerError, UndefinedStackError, UndefinedTableError } from "../../errors";
  ...
  private retrieveImplicitBlinds(): Blind[] {
    ...
    return this.isActives.reduce((blinds, isActive, seat) => {
      ...
      if ((this.isActives.length === 2 && position === 0) || position === 1) {
        blinds.push(this.repos.blind
          .create(seat, BlindType.PostBlind, bigBlind));
      } else if (
        (this.isActives.length === 2 && position === 1) ||
        position === 0
      ) {
        blinds.push(this.repos.blind
          .create(seat, BlindType.PostBlind, smallBlind));
      }
      return blinds;
    }, <Blind[]>[]);
  }
